...
describe('concat', () => {
  ...
  it('should concat apple with banana', () => {
    const result = concat('apple', 'banana')
    expect(result).to.equal('applebanana')
  });

  it('should throw an error if inputs are empty', () => {
    expect(() => { concat('','') }).to.throw('either of the strings are empty')
  });
